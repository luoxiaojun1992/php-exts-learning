...
if test "$PHP_EPOLL" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $EPOLL_DIR/$PHP_LIBDIR, EPOLL_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_EPOLLLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong epoll lib version or lib not found])
  dnl ],[
  dnl   -L$EPOLL_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(EPOLL_SHARED_LIBADD)

  PHP_NEW_EXTENSION(epoll, epoll.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
